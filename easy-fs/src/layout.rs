...
use crate::{block_dev::BlockDevice, BLOCK_SZ, block_cache::block_cache};
...
const INDIRECT1_BOUND: usize = INODE_DIRECT_COUNT;
/// The max size of dir inodes name
const NAME_LENGTH_LIMIT: usize = 27;
/// The size of dir inodes
...
impl DiskInode {
    ...
    pub fn block_id(&self, inner_id: u32, block_device: &Arc<dyn BlockDevice>) -> u32 {
        ...
        if inner_id < INODE_DIRECT_COUNT {
            ...
            block_cache(inner_id as usize, Arc::clone(block_device))
                .lock()
...
impl DirEntry {
    /// Return an empty dir
    pub fn empty() -> Self {
        Self { name: [0u8; NAME_LENGTH_LIMIT + 1], inode_number: 0, }
    }
    ...
    /// Transport inodes in dir to const bytes
    ...
    /// Transport inodes in dir to mutable bytes
    ...
    /// Get the name of inode in dir
    pub fn name(&self) -> &str {
        let len = (0usize..).find(|i| self.name[*i] == 0).unwrap();
        core::str::from_utf8(&self.name[..len]).unwrap()
    }

    /// Get the number of inode in dir
